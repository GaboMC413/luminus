...
export const metadata: Metadata = {
  title: "Para Especialistas | LUMINUS Latam",
  description: "Lleva tu práctica profesional a una red de confianza. Conecta con personas interesadas en su salud integral y expande el alcance de tu acompañamiento.",
};
    ...
    <>
      ...
      <Section id="participar-especialista" borderBottom bg="slate-50">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h2 className="font-display text-3xl font-bold tracking-tight text-slate-900 sm:text-5xl leading-tight">
        ...
        <div className="mx-auto grid max-w-md grid-cols-1 gap-12 md:max-w-5xl md:grid-cols-2 md:gap-8 lg:gap-12 items-stretch">
          {especialistasContent.participation.items.map((item, idx) => (
            <div 
              key={idx} 
              className={`card relative rounded-3xl p-8 md:p-10 flex flex-col justify-between border border-slate-200/80 transition-all duration-300 hover:shadow-medium hover:-translate-y-1 ${
                item.isPopular 
                  ? "luminus-card-glow shadow-medium" 
                  : "bg-white/80 shadow-soft"
              }`}
            >
              {/* Top Badge */}
              {item.isPopular ? (
                <div className="absolute -top-3.5 left-6 flex items-center gap-1.5 rounded-full bg-black px-4 py-1 text-xs font-semibold text-white shadow-soft z-20">
                ...
                item.badgeText && (
                  <div className="absolute -top-3.5 left-6 z-20">
                    <span className="inline-flex items-center rounded-full border bg-slate-50 border-slate-200 px-3.5 py-1 text-xs font-semibold text-slate-600 shadow-soft">
                      {item.badgeText}
                    </span>
                  </div>
                )
              ...
              <div className="flex flex-col justify-between h-full">
                <div>
                  ...
                  <p className={`text-sm leading-relaxed mb-6 font-medium ${item.isPopular ? "text-slate-700" : "text-slate-600"}`}>
                    {item.description}
                  </p>
                  ...
                  <ul className="space-y-4 mb-8">
                    {item.inclusions.map((inclusion, iIdx) => (
                      <li key={iIdx} className="flex items-start gap-3 text-sm text-slate-600 font-medium">
                        <Check className="h-5 w-5 shrink-0 text-emerald-500 stroke-[2px] mt-0.5" />
                        <span>{inclusion}</span>
                      </li>
                    ))}
                  </ul>
                ...
                <div>
                  {item.note && (
                    <p className="text-[11px] leading-relaxed text-slate-400 mb-6 font-medium bg-white/60 p-3 rounded-xl border border-slate-100">
                      {item.note}
                    </p>
                  )}
                  <Button 
      ...
      <Section id="sesiones-introductorias" borderBottom bg="white">
        ...
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:items-center">
          ...
          <div className="lg:col-span-7 text-left flex flex-col items-start">
            ...
            <p className="text-lg leading-relaxed text-slate-800 font-bold mb-6">
              {especialistasContent.introSessions.subtitle}
            </p>

            <div className="space-y-4 text-sm sm:text-base leading-relaxed text-slate-600 font-medium mb-8">
          ...
          <div className="lg:col-span-5 flex justify-center">
            <div className="card w-full max-w-[400px] rounded-3xl border border-slate-200/80 bg-white/80 p-6 shadow-soft transition-all duration-300 hover:shadow-medium relative overflow-hidden">
              {/* Badge representing Plus */}
              <div className="absolute top-4 right-4 flex items-center gap-1 rounded-full bg-black px-3 py-0.5 text-[10px] font-semibold text-white shadow-soft">
              ...
              <div className="flex items-center gap-4 mb-6">
                <div className="h-14 w-14 rounded-full border border-slate-100 bg-[#FFE0FC] flex items-center justify-center font-display text-lg font-bold text-[#B832B4] shadow-soft shrink-0">
                  SL
                </div>
                <div>
                  <h4 className="font-display font-bold text-slate-900 leading-tight">Dr. Sofía López</h4>
                  <p className="text-xs text-slate-400 font-semibold">Terapeuta & Coach de Vida</p>
                  <div className="flex items-center gap-1 mt-1">
                    <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
                    <span className="text-[10px] text-emerald-600 font-semibold">Sesión de 15 min disponible</span>
                  </div>
                </div>
              </div>
                  ...
                  {["10:00 AM", "11:30 AM", "15:15 PM", "16:00 PM", "17:30 PM", "18:45 PM"].map((timeSlot, tsIdx) => (
                    <button
                      ...
                      className={`py-2 px-1 text-xs font-semibold rounded-xl border transition-all duration-200 ${
                        tsIdx === 2
                          ? "bg-[#F4F8B8] text-[#7A8500] border-[#D4E600]/30 shadow-soft"
              ...
              {/* Callout box */}
              <div className="bg-slate-50/50 rounded-2xl p-4 border border-slate-100 flex items-start gap-3">
                <Clock className="h-5 w-5 text-[#FF7700] shrink-0 mt-0.5 animate-pulse" />
                <div>
                  <h5 className="text-xs font-bold text-slate-900">Conexión Express 100% gratuita</h5>
      ...
      <Section id="funciones-especialistas" borderBottom bg="slate-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {especialistasContent.features.items.map((feat, idx) => (
            <div key={idx} className="card flex flex-col justify-between rounded-3xl border border-slate-200/80 bg-white/80 p-8 shadow-soft hover:shadow-medium hover:-translate-y-1 transition-all duration-300 group">
              <div>
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-slate-100 bg-slate-50 text-slate-700 shadow-soft mb-6 transition-transform duration-300 group-hover:scale-105">
                  {feat.icon}
                </div>
              ...
              <div>
                <a 
                  href={feat.ctaLink} 
                  className="inline-flex items-center gap-1 text-xs font-bold text-slate-800 hover:text-black transition-colors group/link"
                >
      ...
      <Section borderBottom bg="white" className="relative py-28 text-center overflow-hidden">
        ...
        <Container>
          <div className="max-w-4xl mx-auto">
            ...
            <div className="w-16 h-1 bg-[#FF7700] mx-auto rounded-full mb-8" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left max-w-3xl mx-auto text-sm sm:text-base leading-relaxed text-slate-600 font-medium">
              <p>
      ...
      <Section borderBottom bg="slate-50">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-5 order-2 lg:order-1 flex justify-center animate-fadeIn">
            <div className="card rounded-3xl border border-slate-200/80 bg-[#F4F8B8]/30 p-8 shadow-soft max-w-[380px] w-full text-center transition-all duration-300 hover:shadow-medium hover:-translate-y-1">
              <div className="flex h-14 w-14 mx-auto items-center justify-center rounded-2xl border border-slate-100 bg-[#D4E600] text-[#7A8500] shadow-soft mb-6">
                <Shield className="h-7 w-7" />
              </div>
              <h3 className="font-display text-xl font-bold text-slate-900 mb-4">
                100% Independiente
              </h3>
              <p className="text-xs sm:text-sm leading-relaxed text-slate-600 font-medium mb-6">
              ...
              <div className="flex items-center justify-center gap-2 border border-dashed border-slate-200 p-3 rounded-2xl bg-white/80 shadow-soft">
                <Sparkles className="h-4 w-4 text-[#FF7700] shrink-0 animate-pulse" />
          ...
          <div className="lg:col-span-7 order-1 lg:order-2 text-left animate-fadeIn">
      ...
      <Section id="como-funciona" borderBottom bg="white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 mb-16">
          {especialistasContent.stepsSection.steps.map((step, idx) => {
            ...
            if (idx === 0) accentBorderClass = "border-t-4 border-[#0450FB]";
            else if (idx === 1) accentBorderClass = "border-t-4 border-[#D4E600]";
            else if (idx === 2) accentBorderClass = "border-t-4 border-[#FF7700]";
            else if (idx === 3) accentBorderClass = "border-t-4 border-[#FF80FC]";
            else accentBorderClass = "border-t-4 border-[#0450FB]";

            return (
              <div 
                key={idx}
                className={`card relative flex flex-col justify-between rounded-3xl border border-slate-200/80 bg-white/80 p-6 shadow-soft hover:shadow-medium hover:-translate-y-1 transition-all duration-300 group ${accentBorderClass}`}
              >
                {/* Number indicator */}
                <div className="flex items-baseline justify-between mb-6">
                  <span className="font-display text-4xl font-bold tracking-tight text-slate-300 transition-transform duration-300 group-hover:scale-105">
                    {step.num}
                  </span>
                  <span className="inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-semibold border bg-slate-50 text-slate-500 border-slate-200/60">
                    Paso {step.num}
                  </span>
                </div>
      ...
      <Section id="comenzar-camino" bg="white" borderBottom className="py-24 relative overflow-hidden">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[350px] bg-gradient-to-tr from-[#FFE0C2]/15 to-transparent rounded-full blur-3xl pointer-events-none opacity-40" />
            ...
            <div className="relative z-10 max-w-3xl mx-auto flex flex-col items-center">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4 w-full sm:w-auto">
                <Button 
                  ...
                  className="w-full sm:w-auto px-8 py-3.5 border border-black text-base font-semibold shadow-soft hover:shadow-medium hover:bg-neutral-900 animate-fadeIn"
                >
                ...
                <Button 
                  ...
                  className="w-full sm:w-auto px-8 py-3.5 border border-slate-200 bg-white text-slate-800 font-semibold shadow-soft hover:shadow-medium hover:bg-slate-50 animate-fadeIn"
                >
      ...
      <section className="bg-slate-50/50 py-8 border-b border-slate-100">
        <Container className="text-center">
          <p className="max-w-4xl mx-auto text-[11px] leading-relaxed text-slate-400 font-medium text-justify sm:text-center px-4">
