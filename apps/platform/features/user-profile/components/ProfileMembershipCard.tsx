...
export function ProfileMembershipCard({ plan, createdAt }: ProfileMembershipCardProps) {
  ...
  return (
    <div className="bg-white rounded-[16px] p-5 border border-slate-200 flex flex-col gap-5 shadow-none relative group">
      <div className="w-full flex items-start gap-4">
        <div className="mt-1">
          <span className="material-symbols-rounded text-muted text-[20px]">award_star</span>
        </div>
        <div className="flex flex-col min-w-0 self-center">
          <span className="text-body text-secondary text-primary truncate">Membresía {plan || 'Free'}</span>
