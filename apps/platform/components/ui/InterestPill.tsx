...
export const InterestPill = ({ interest, className = "" }: InterestPillProps) => {
  ...
  return (
    <div 
      className={`h-9 px-4 rounded-full border flex justify-center items-center transition-all hover:brightness-95 shrink-0 ${className}`}
      style={{
    ...
    >
      <span className="text-[12px] md:text-[14px] font-medium text-center line-clamp-1 truncate font-sans select-none">
        {interest}
      </span>
    </div>
  );
