      ...
      <div className="flex flex-col gap-1 min-w-0 flex-1 justify-center">
        <div className="flex flex-col gap-0.5">
          <div className="flex items-center justify-between gap-2">
            <span className="text-sm font-semibold text-slate-900 truncate leading-tight">{user}</span>
        ...
        {buttonLabel && (
          <button
            ...
            className="w-fit h-8 px-4 bg-black hover:bg-slate-800 text-white text-xs font-semibold rounded-full transition-all mt-1"
          >
            {buttonLabel}
          </button>
        )}
  ...
  return (
    ...
    >
      ...
      <div className="max-h-[380px] overflow-y-auto custom-scrollbar">
        {messages.length === 0 ? (
          <div className="p-8 text-center text-slate-400 text-sm">No tienes mensajes pendientes</div>
        ) : (
