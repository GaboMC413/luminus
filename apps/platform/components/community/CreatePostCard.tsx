...
export function CreatePostCard({ currentUserProfile, onAddPost }: CreatePostCardProps) {
  ...
  const handleEditorInput = () => {
    ...
    // Disallow and strip any images pasted or inserted directly into the text area
  ...
  const handleEditorPaste = (e: React.ClipboardEvent<HTMLDivElement>) => {
    // 1. Block pasting direct image files from clipboard
    ...
    // 2. If pasting HTML (e.g. copied from another webpage), strip any <img> or media elements
    ...
    if (html) {
      ...
      // Strip inline styles and classes to enforce predefined platform typography & colors
  ...
  const userName = currentUserProfile
    ? `${currentUserProfile.first_name || ""} ${currentUserProfile.last_name || ""}`.trim() || "Usuario"
    : "Usuario";
  ...
  const handleImageFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // 3. Upload image in background with progress tracking
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const allAreas = selectedAreas.length > 0 ? [selectedCategory, ...selectedAreas] : [selectedCategory];

    const formatCityCountry = (loc?: string) => {
      if (!loc) return "";
    ...
    let userLocation = "Montevideo, Uruguay";
    if (userCity && userCountry) {
      userLocation = `${userCity}, ${userCountry}`;
    } else if (currentUserProfile?.location) {
      userLocation = formatCityCountry(currentUserProfile.location);
    ...
    const newPost: PostItem = {
      id: `post-${Date.now()}`,
      authorId: currentUserProfile?.id || currentUserProfile?.user_id || "current-user",
  ...
  return (
    <>
      {/* 1. Trigger Button: Same family as the square action buttons */}
      <button
        type="button"
        onClick={() => setIsModalOpen(true)}
        className="w-full h-11 sm:h-12 px-4 rounded-2xl bg-white border border-zinc-200/80 hover:border-slate-400 hover:bg-slate-50/80 text-left transition-all duration-200 active:scale-[0.99] cursor-pointer flex items-center justify-between group outline-none select-none"
        title="Crear una publicación"
      >
        <span className="text-[14px] sm:text-[15px] font-normal text-slate-500 font-sans tracking-normal">
          Crear una publicación
        </span>
        <span className="material-symbols-outlined text-[20px] text-slate-500 group-hover:text-slate-800 transition-colors shrink-0">
          add
        </span>
      </button>

      {/* 2. Main Modal: Modern Minimalist Composer */}
      <Modal
        isOpen={isModalOpen}
        onClose={handleResetAndCloseModal}
        title="Comparte algo con la comunidad."
        subtitle="Revisamos cada publicación para cuidar la calidad del espacio."
        hideDividers
        disableInnerScroll
        maxWidth="620px"
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden flex flex-col focus-within:border-slate-400 transition-colors">
            ...
            <div className="px-4 py-3 border-t border-slate-100 flex flex-col gap-2.5 bg-white">
              <div className="flex items-center justify-between">
                ...
                >
                  <svg className="w-4 h-4 text-slate-700 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  {selectedCategory && (
                    <svg
                      ...
                      viewBox="0 0 10 6"
                      fill="none"
                      className={`transition-transform duration-200 text-slate-400 ${showTagPills ? "rotate-180" : ""}`}
                    >
                  ...
                  {activeCategoryObj.items.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`h-7 px-3 rounded-full text-xs font-medium transition-all cursor-pointer select-none font-sans flex items-center gap-1.5 shrink-0 border ${isSelected
          ...
          {imagePreview && (
            <div className="relative rounded-2xl overflow-hidden bg-zinc-100 border border-zinc-200/80">
              <img src={imagePreview} alt="Vista previa" className="w-full max-h-[240px] object-cover" />

              {/* Uploading overlay badge with clean progress bar */}
              {isUploadingImage && (
              ...
              {/* Ready badge when upload finishes */}
              {!isUploadingImage && uploadedImageUrl && (
                <div className="absolute bottom-2.5 left-2.5 px-2.5 py-1 rounded-full bg-zinc-900/80 backdrop-blur-md text-white text-[11px] font-medium flex items-center gap-1.5">
          ...
          {imageError && (
            <div className="px-3.5 py-2.5 rounded-xl bg-rose-50 border border-rose-200/80 text-rose-800 text-xs font-sans flex items-start gap-2 animate-in fade-in duration-150">
              <span className="material-symbols-outlined text-[16px] shrink-0 text-rose-500 mt-0.5">error</span>
              <div className="flex-1 flex flex-col gap-0.5">
                <span className="font-semibold font-jakarta text-rose-900">No se pudo cargar la imagen</span>
          ...
          <div className="pt-2 grid grid-cols-2 gap-2.5 sm:gap-3 w-full">
            ...
            >
              <svg className="w-4.5 h-4.5 sm:w-5 sm:h-5 text-slate-700 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* 3. Submodal: Agregar video de YouTube */}
      <Modal
