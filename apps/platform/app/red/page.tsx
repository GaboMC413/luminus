      ...
      <div className="flex-1 w-full max-w-7xl mx-auto px-4 md:px-6 py-4 md:py-6 flex flex-col min-h-0">
        <div className="w-full max-w-6xl mx-auto flex flex-col flex-1 min-h-0">
          ...
          <div className="flex items-center gap-3 mb-4 md:mb-6 shrink-0">
            ...
            <h1 className="text-xl text-black font-semibold font-jakarta">Mi red</h1>
          </div>
          ...
          {/* Main Layout Grid */}
          <div className="flex flex-col md:grid md:grid-cols-12 gap-4 md:gap-6 flex-1 min-h-0 items-start overflow-y-auto md:overflow-hidden pb-12 md:pb-0">
            
            {/* Left Column: Mi red / active connections (col-span-8) */}
            <div className="order-2 md:order-1 md:col-span-8 flex flex-col bg-white rounded-2xl border border-slate-200 overflow-hidden h-fit md:h-full min-h-0">
              ...
              <div className="flex-1 md:overflow-y-auto md:custom-scrollbar divide-y divide-slate-100 bg-white">
                {isLoading && connections.length === 0 ? (
                  <div className="p-6 text-[14px] text-slate-400 text-center">Cargando tu red...</div>
                ) : filteredNetwork.length === 0 ? (
                  <div className="p-12 text-center text-slate-400 text-[13px] flex flex-col items-center gap-3 py-16">
                      ...
                      >
                        ...
                        <div className="relative shrink-0">
                          {conn.user.avatar ? (
                            ...
                            <div className="w-12 h-12 rounded-[8px] bg-slate-100 flex items-center justify-center text-slate-400">
                              <span className="material-symbols-rounded text-[26px]">person</span>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="text-[14px] font-bold text-slate-900 truncate font-jakarta group-hover/line:underline leading-snug">
                            {conn.user.name}
                          </h4>
            ...
            <div className="order-1 md:order-2 md:col-span-4 flex flex-col bg-white rounded-2xl border border-slate-200 overflow-hidden h-fit md:h-full min-h-0 shrink-0">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar divide-y divide-slate-100 md:max-h-full max-h-[300px]">
                {isLoading && connections.length === 0 ? (
                  ...
                  <div className="p-8 text-center text-slate-400 text-[13px] flex flex-col items-center gap-3 py-12">
                    <span className="material-symbols-rounded text-slate-300 text-[32px] select-none">person_add_disabled</span>
                    <span>No tienes solicitudes de conexión pendientes</span>
                  </div>
                ) : (
                    ...
                    <div key={conn.id} className="flex items-center gap-3 p-4 bg-white hover:bg-slate-50/50 transition-colors">
                      ...
                      >
                        ...
                        <div className="relative shrink-0">
                          {conn.user.avatar ? (
                            ...
                            <div className="w-11 h-11 rounded-[8px] bg-slate-100 flex items-center justify-center text-slate-400">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button
                          ...
                          className="h-8 px-3.5 bg-black hover:bg-slate-800 text-white rounded-lg text-[10px] font-bold transition cursor-pointer font-jakarta uppercase tracking-wider flex items-center justify-center shrink-0"
                        >
                          Aceptar
                        </button>
                        <button
